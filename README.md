👋 Muhammad Okfriansyah (Opi)

Engineering Manager · Backend Systems · Distributed Systems · AI Systems Builder

I build systems that run in production, not just in slides

🧠 About Me
Engineering Manager @ Mekari
Focused on backend architecture, distributed systems, and AI-driven pipelines
Strong bias toward deterministic, observable, and scalable systems
Builder of autonomous systems that generate real output (content, signals, revenue)
⚙️ Technical Stack
Languages      : Go, Node.js, Java
Backend        : REST, gRPC, event-driven systems
Database       : PostgreSQL, Redis
Infra          : Docker, Linux, CI/CD (Bitbucket/GitHub Actions)
Architecture   : Distributed systems, queue-based processing, state machines
AI Layer       : LLM integration, autonomous pipelines, prompt systems
🧩 System Design Principles
- Prefer simplicity over flexibility
- Single orchestrator > distributed coordination
- Idempotent operations everywhere
- DB-backed state machines > implicit state
- Cron-driven pipelines > complex schedulers
- Observability is not optional

If failure modes are unclear, the system is not production-ready.

🚧 Key Projects
🔹 MD-AME (Autonomous Content Engine)
End-to-end AI pipeline:
Input → Clip extraction → Processing → Publishing
Multi-channel YouTube system
Designed for automation + monetization at scale
Focus:
Deterministic workflows
High-throughput processing
Minimal human intervention
🔹 KOC Engine (Production Architecture Locked)
Deterministic content production system
Designed for:
Repeatability
Reliability
Scale
No experimentation layer — execution only
🔹 Market Intelligence Assistant (Planned)
Multi-asset scanner:
Crypto
US stocks
Indonesian stocks
Features:
Signal generation
Portfolio tracking
Whale + liquidity tracking
Telegram/WhatsApp alerts

Goal:

Turn data into actionable decisions, not dashboards

💰 Investing Strategy
Style        : High-risk, high-return
Focus        : Asymmetric bets, low market cap, multibaggers
Approach     : Long-term + selective swing
Constraint   : Discipline > emotion

Avoid:

Narrative-driven entries
Overtrading
No clear exit plan
📊 Engineering Leadership

I optimize teams for:

High deployment frequency
Low lead time
Fast recovery (MTTR)
Controlled failure rate

Frameworks:

OKRs
DORA metrics
Continuous delivery mindset
📈 GitHub Stats
<p align="center"> <img src="https://github-readme-stats.vercel.app/api?username=okfriansyah-moh&show_icons=true&hide_border=true" /> <img src="https://github-readme-streak-stats.herokuapp.com/?user=okfriansyah-moh&hide_border=true" /> </p>
🔗 Reach Me
LinkedIn:
https://www.linkedin.com/in/muhammad-okfriansyah-74092671/
⚡ Operating Principle

Build → Test → Observe → Iterate

🧭 What I’m Building Toward
Autonomous systems that:
Generate revenue
Scale without linear effort
Operate with minimal human input
